...
1. RAG 기반 업무 매뉴얼과 안전한 AI 조회 도구
2. Docker·AWS 배포와 장애 알림 자동화
3. 대량 파이프라인 처리량·실패 알림 관측성 강화
...
같은 DAG 실행 안에서는 원본 주문상품 ID를 한 번만 staging에 저장하고, fact는 원본 주문상품 ID를 고유키로 사용합니다. 따라서 서로 다른 백필 실행은 각각 검증 이력을 남기면서도 판매 사실은 중복 적재하지 않습니다. UPSERT로 일별 mart를 다시 계산하므로 같은 기간을 반복 실행해도 행 수와 매출 합계가 늘어나지 않습니다.

### 백필과 오류 재처리

관리자는 `배치 작업` 화면에서 최대 31일 범위의 과거 시작·종료 시각을 선택해 Airflow 백필을 요청할 수 있습니다. Spring 서버가 Airflow API에 인증한 뒤 DAG run을 만들고, 요청자·기간·DAG run ID를 `audit_logs`에 기록합니다. 조회는 운영자·승인자·관리자가 가능하지만 실행은 `ADMIN`만 가능합니다.

검증에 실패한 주문상품은 삭제하지 않고 `rejected_order_items`에 `PENDING`으로 격리합니다. 상품이나 SKU 기준정보를 수정한 뒤 `미해결 오류도 다시 검증`을 켜고 실행하면 기간 밖의 미해결 행도 다시 staging으로 가져옵니다. 이번 실행에서 검증을 통과한 행만 `RESOLVED`로 바꾸므로, 과거 fact가 존재한다는 이유만으로 오류가 해결 처리되지 않습니다.

로컬 검증에서는 SKU를 일시 중지해 `INACTIVE_SKU` 오류를 만들고, 일반 백필 뒤에는 `PENDING`이 유지되는 것을 확인했습니다. SKU를 복구하고 오류 재처리를 실행하자 `RESOLVED`로 변경됐으며, 같은 기간을 여러 번 처리한 뒤에도 fact 3행과 총매출 354,000원이 그대로 유지됐습니다.
